...
func (txs Txs) GetInfo(head int, page int, size int) ([]Txs,int){
	...
	if head == 0 {
		var txs Txs
		dbConn.C("Txs").Find(nil).Sort("-height").One(&txs)
		head = txs.Height
	}
	dbConn.C("Txs").Find(bson.M{"height": bson.M{
...
func (txs Txs) GetDetail(txhash string) Txs {
	...
	dbConn := session.DB(conf.NewConfig().DBName)
	var temptxs Txs
	dbConn.C("Txs").Find(bson.M{"txhash": txhash}).One(&temptxs)
	return temptxs
}
...
func (txs Txs) GetPowerEventInfo(address string, page, size int) (*[]Txs, int) {
	...
	count, _ := dbConn.C("Txs").Find(bson.M{"$or": query, "validatoraddress": bson.M{"$elemMatch": bson.M{"$eq": address}}}).Count()
	_ = dbConn.C("Txs").Find(bson.M{"$or": query, "validatoraddress": bson.M{"$elemMatch": bson.M{"$eq": address}}}).Sort("-height").Limit(size).Skip(page * size).All(&txsSet)
	return &txsSet, count
}
func (txs Txs) GetDelegatorTxs(address string, page, size int) (*[]Txs, int) {
	...
	query = append(query, q1, q2, q3,q4)
