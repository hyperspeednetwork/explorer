...
func (vi ValidatorInfo) SetInfo(log zap.Logger) {
	...
	_, err := c.Upsert(bson.D{{"validatoraddress", vi.ValidatorAddress}}, &vi)
	//_,err := c.Upsert(bson.D{{"validatoraddress", vi.ValidatorAddress}}, &vi)
	if err == nil {
		log.Info("insert validatorInfo success")
	} else {
		log.Error("insert validatorInfo failed", zap.String("error", err.Error()))
	}
	defer log.Sync()
}
...
func (vi ValidatorInfo) GetValidatorRank(amount float64, jailed bool) int {
	session := db.NewDBConn()
	defer session.Close()
	dbConn := session.DB(conf.NewConfig().DBName)
	if !jailed {
		rank, _ := dbConn.C("validators").Find(bson.M{
